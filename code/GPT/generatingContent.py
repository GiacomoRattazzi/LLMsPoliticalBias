...
def getResultsLLM(generation_type, model):
    ...
    if generation_type == 0:
        ...
        raise ValueError("Invalid generation_type.")
    ...
    create_csv(f'{gen_type}_individual_scores_LEFT.csv', ['Prompt', 'Generated Content', 'BERT Score', 'Predicted Stance'],
               individual_scores)
    batched_scores = batch_and_score(generated_contents)
    create_csv(f'{gen_type}_batched_bert_scores_LEFT.csv', ['Number of Prompts', 'BERT Score', 'Predicted Stance'], batched_scores)

    total_time = (time.time() - start_time)/60
    print(f"Total time taken: {total_time:.2f} minutes")

if __name__ == '__main__':
    getResultsLLM(0, 'gptModel')
